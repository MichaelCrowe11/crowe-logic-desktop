...
(async()=>{
 ...
 await test('previous list states approval plainly for later proposers',async()=>{const f=fixture({spec:{maxSteps:2,maxCalls:12}});await C.run(f.state,f.deps);const proposes=f.requests.filter(r=>r.task==='propose');assert.equal(proposes.length,2);const prior=proposes[1].data.previous[0];assert.equal(prior.status,'verified');assert.equal(prior.approved,true);assert.equal(prior.outcome,'approved, executed and verified');assert.equal(proposes[0].data.previous.length,0);});
