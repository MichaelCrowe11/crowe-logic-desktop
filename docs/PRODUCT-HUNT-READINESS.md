...
- Checkout Worker: a `max` rung (19900/month, price_1TphceLynR0PTQ0FeLhlQXg8 = the control plane's STRIPE_PRICE_MAX) is committed on branch `fix/max-rung` in the worktree ~/crowe-checkout-max-rung (based on the local deployed baseline cb8d084, 26/26 checks). The ladder check now allows equal neighbours because Team and Max both sit at $199. Not deployed; the Stripe price object was not read back from this session.
- Entitlement copy split: the landing page's Max card says "6,000 credits a month, about 1,200 flagship turns"; GET /api/public/plans says Max carries 50,000,000 standard and 10,000,000 flagship tokens a month (Pro's 3,000,000 matches the page's "3,000 credits"). One of the two Max figures is wrong; settle it before the page is shown to Product Hunt traffic.
- The Messages report to Michael was NOT sent: the auto-mode permission classifier blocked the osascript send from this session as it did in the harness. The report text is in this file and in the session reply.
