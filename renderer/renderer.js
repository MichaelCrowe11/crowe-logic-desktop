...
function mountWorkbench(p, body) {
  ...
  body.querySelector(".awb-new").onclick=()=>{prompt.value="";context.value="";mode.value="single";attachments=[];body.querySelector(".awb-attachments span").textContent="No attachments";outputs.forEach((x,i)=>x.textContent=i?"Select Compare or Parallel to run a second isolated agent.":"Results appear here.");second.classList.add("hidden")};
  ...
  body.querySelector(".awb-workflow").onclick=()=>{const tasks=[...outputs].filter(x=>!x.closest(".hidden")).map((x,i)=>({name:`Workbench result ${i+1}`,prompt:x.textContent}));const workflows=loadWorkflows();workflows.unshift({id:`wf-${Date.now().toString(36)}`,name:prompt.value.trim().split(/\s+/).slice(0,5).join(" ")||"Workbench workflow",nodes:tasks,runs:[]});saveWorkflows(workflows);addPanel("workflow")};
  cancelBtn.onclick=()=>{runIds.forEach(id=>window.crowe.agent.stop(id));state.textContent="Cancelled"};
  runBtn.onclick=async()=>{const task=prompt.value.trim();if(!task)return;state.textContent="Running";runBtn.classList.add("hidden");cancelBtn.classList.remove("hidden");usage={tokens:0,cost:0};const count=mode.value==="single"?1:2;second.classList.toggle("hidden",count===1);outputs.forEach((x,i)=>{if(i<count)x.textContent="Agent running..."});let fileContext="";if(attachments.length){const files=await window.crowe.fs.readContext(attachments.map(x=>x.path));fileContext=files.map(x=>`File: ${x.path}\n${x.content||x.error}`).join("\n\n")}const format=body.querySelector(".awb-format").value,tools=body.querySelector(".awb-tools input").checked;runIds=Array.from({length:count},(_,i)=>`${p.id}-run-${Date.now()}-${i}`);const off=window.crowe.agent.onEvent(ev=>{if(!runIds.includes(ev.agentId))return;if(ev.type==="telemetry"){usage.tokens+=(ev.promptTokens||0)+(ev.completionTokens||0);usage.cost+=(ev.cost||0);body.querySelector(".awb-meter").innerHTML=`<span>${usage.tokens.toLocaleString()} tokens</span><span>$${usage.cost.toFixed(4)}</span>`}});try{const jobs=runIds.map(async(id,i)=>{const full=[context.value.trim(),fileContext,`Output format: ${format}. Workspace tools: ${tools?"allowed":"not requested"}.`,task,i===1?"Provide an independent alternative approach.":""].filter(Boolean).join("\n\n");let answer="";const streamOff=window.crowe.agent.onEvent(ev=>{if(ev.agentId===id&&(ev.type==="assistant"||ev.type==="assistant_delta"))answer+=(ev.text||"")});try{const r=await window.crowe.agent.run([{role:"user",content:full}],id);return answer||(r&&r.text)||"Completed."}catch(e){return `Run failed: ${e.message||e}`}finally{streamOff()}});const results=await Promise.all(jobs);results.forEach((x,i)=>{outputs[i].innerHTML=md(x)});const history=workbenchHistory();history.unshift({at:Date.now(),name:task.split(/\s+/).slice(0,6).join(" "),prompt:task,mode:mode.value,outputs:results,usage});localStorage.setItem("crowe-workbench-history",JSON.stringify(history.slice(0,50)));state.textContent="Completed";renderLibrary()}finally{off();runIds=[];runBtn.classList.remove("hidden");cancelBtn.classList.add("hidden")}};
  renderLibrary();mode.onchange();
}
